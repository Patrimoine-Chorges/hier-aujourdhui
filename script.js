...
// V10.2 — Lissage des capteurs pour éviter les indications instables.
const ORIENTATION_SMOOTHING = 0.22;
let smoothedOrientation = { azimut: null, pitch: null, roll: null };
let alignmentVibrationDone = false;

function smoothLinear(previous, next, factor = ORIENTATION_SMOOTHING) {
  if (!Number.isFinite(next)) return previous;
  if (!Number.isFinite(previous)) return next;
  return previous + (next - previous) * factor;
}

function smoothHeading(previous, next, factor = ORIENTATION_SMOOTHING) {
  if (!Number.isFinite(next)) return previous;
  if (!Number.isFinite(previous)) return normalize360(next);
  const delta = signedAngleDifference(previous, next);
...
function evaluateGuidance() {
  ...
  // Tolérance volontairement confortable pour le grand public.
  // Une valeur personnalisée dans points.json reste possible, avec un minimum de 15°.
  const azTol = Math.max(15, Number(settings.toleranceAzimut || 15));
  const azDistance = Math.abs(azDiff);

  if (azDistance > azTol) {
    alignmentMessageShown = false;
    alignmentVibrationDone = false;
    const direction = azDiff > 0 ? "à droite" : "à gauche";
    const strongTurn = azDistance > 30;
    setGuidance(
      strongTurn
  ...
  showAlignedMessage();
  if (!alignmentVibrationDone && navigator.vibrate) {
    navigator.vibrate(120);
    alignmentVibrationDone = true;
  }
}
...
function handleOrientation(event) {
  ...
  if (typeof event.webkitCompassHeading === "number") {
    rawAzimut = event.webkitCompassHeading;
  } else if (typeof event.alpha === "number") {
    rawAzimut = normalize360(360 - event.alpha);
  }

  const rawPitch = typeof event.beta === "number" ? event.beta : null;
  const rawRoll = typeof event.gamma === "number" ? event.gamma : null;

  smoothedOrientation.azimut = smoothHeading(smoothedOrientation.azimut, rawAzimut);
  smoothedOrientation.pitch = smoothLinear(smoothedOrientation.pitch, rawPitch);
  smoothedOrientation.roll = smoothLinear(smoothedOrientation.roll, rawRoll);
