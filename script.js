...
const closeAppBtn=document.getElementById("closeAppBtn");if(closeAppBtn){closeAppBtn.addEventListener("click",()=>{if(history.length>1){history.back();return;}window.close();setTimeout(()=>{if(!document.hidden)alert("Vous pouvez maintenant fermer cette page.");},300);});}
