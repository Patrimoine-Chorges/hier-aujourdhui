...
function setComparisonMode(enabled) {
  ...
  adjustBtn.textContent = enabled ? "✥ Ajuster" : "✓ Comparer";
...
function setGuidanceVisible(visible) {
  ...
  if (guidanceBtn) {
    guidanceBtn.textContent = visible ? "📍 Masquer" : "📍 Guidage";
...
guidanceBtn?.addEventListener("click", event => {
  ...
  event.stopPropagation();
  const box = getGuidanceBox();
  const visible = box && !box.classList.contains("guidance-hidden");
...
document.addEventListener("fullscreenchange", () => {
  ...
  if (fullscreenBtn) {
    fullscreenBtn.textContent = active ? "✕ Quitter" : "⛶ Plein écran";
  }
...
window.addEventListener("load", () => { setGuidanceVisible(true); showControls(); });
