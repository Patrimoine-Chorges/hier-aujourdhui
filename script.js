...
oldPhoto.addEventListener("pointerdown", event => {
  if (!photoViewOpen || event.pointerType === "touch") return;
...
/* =========================================================
   V11.27 — ZOOM TACTILE ROBUSTE (iPhone / Android)
   Les gestes tactiles utilisent directement Touch Events.
   Cela évite les différences de gestion des Pointer Events
   selon Safari / Chrome mobile.
   ========================================================= */
...
oldPhoto.addEventListener("touchstart", event => {
  ...
  touchGestureStart = {
    ...
    center: touches.length >= 2 ? center(touches[0], touches[1]) : touches[0],
    distance: touches.length >= 2 ? distance(touches[0], touches[1]) : 0,
    angle: touches.length >= 2 ? angle(touches[0], touches[1]) : 0
  };
}, { passive: false });

oldPhoto.addEventListener("touchmove", event => {
  if (!photoViewOpen || !touchGestureStart) return;
  event.preventDefault();
  event.stopPropagation();
  ...
  if (touches.length === 1) {
    const t = touches[0];
    state.x = touchGestureStart.state.x + (t.clientX - touchGestureStart.center.clientX);
    state.y = touchGestureStart.state.y + (t.clientY - touchGestureStart.center.clientY);
  } else {
    const a = touches[0];
    const b = touches[1];
    ...
    const c = center(a, b);
    const ang = angle(a, b);
    const baseDistance = Math.max(1, touchGestureStart.distance || d);

    state.scale = Math.max(0.5, Math.min(5, touchGestureStart.state.scale * (d / baseDistance)));
    state.rotation = touchGestureStart.state.rotation + (ang - touchGestureStart.angle);
    state.x = touchGestureStart.state.x + (c.x - touchGestureStart.center.x);
    state.y = touchGestureStart.state.y + (c.y - touchGestureStart.center.y);
  }

  applyTransform();
}, { passive: false });

oldPhoto.addEventListener("touchend", event => {
  if (!photoViewOpen) return;
  event.preventDefault();
  event.stopPropagation();
...
oldPhoto.addEventListener("touchcancel", () => {
  touchGestureStart = null;
}, { passive: false });
...
async function exitFullscreenMode() {
  ...
  document.body.classList.remove("pseudo-fullscreen", "is-fullscreen", "controls-hidden");
  if (fullscreenBtn) {
    fullscreenBtn.classList.remove("is-fullscreen-active");
    fullscreenBtn.setAttribute("aria-label", "Passer en plein écran");
...
["closeAppBtn", "closeWelcomeBtn"].forEach(id => {
