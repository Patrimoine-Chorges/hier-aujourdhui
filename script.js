...
let comparisonStateBeforePhotoView = null;
...
oldPhoto.addEventListener("pointermove", event => {
  ...
  if (pts.length >= 2) {
    ...
    state.rotation = photoViewOpen ? 0 : startState.rotation + (newAngle - startAngle);
...
function setPhotoView(open) {
  const shouldOpen = Boolean(open) && photoAccessGranted;

  if (shouldOpen && !photoViewOpen) {
    comparisonStateBeforePhotoView = { ...state };
    state = { ...state, x: 0, y: 0, scale: 1, rotation: 0, opacity: 1 };
  } else if (!shouldOpen && photoViewOpen && comparisonStateBeforePhotoView) {
    state = { ...comparisonStateBeforePhotoView };
    comparisonStateBeforePhotoView = null;
  }

  photoViewOpen = shouldOpen;
  pointers.clear();
