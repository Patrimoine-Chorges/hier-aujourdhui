...
const tabFr = document.getElementById("tabFr");
const tabEn = document.getElementById("tabEn");

let textByLanguage = { fr: "", en: "" };
let currentTextLanguage = "fr";
...
function setAudioButtonState(playing) {
  ...
  audioBtn.setAttribute("aria-label", playing ? "Arrêter la voix off" : "Écouter la voix off");
  audioBtn.title = playing ? "Arrêter la voix off" : "Écouter la voix off";
}
...
async function toggleVoiceOver() {
  ...
  try {
    voiceOver.src = assetUrl(`audio/V${photoNumber}.mp3`);
    voiceOver.currentTime = 0;
...
function parseBilingualText(rawText) {
  ...
  if (paragraphs.length >= 2) {
    return { fr: paragraphs[0], en: paragraphs.slice(1).join("\n\n") };
  }
  const lines = cleaned.split("\n").map(line => line.trim()).filter(Boolean);
  const midpoint = Math.ceil(lines.length / 2);
  return {
    fr: lines.slice(0, midpoint).join("\n"),
    en: lines.slice(midpoint).join("\n")
  };
}

function renderTextLanguage(language) {
  currentTextLanguage = language === "en" ? "en" : "fr";
  tabFr?.classList.toggle("active", currentTextLanguage === "fr");
  tabEn?.classList.toggle("active", currentTextLanguage === "en");
  tabFr?.setAttribute("aria-selected", String(currentTextLanguage === "fr"));
  tabEn?.setAttribute("aria-selected", String(currentTextLanguage === "en"));
  if (textContent) {
    textContent.textContent = textByLanguage[currentTextLanguage] ||
      (currentTextLanguage === "fr" ? "Texte indisponible." : "Text unavailable.");
    textContent.scrollTop = 0;
  }
}

async function loadTextForCurrentView() {
  try {
    const response = await fetch(assetUrl(`textes/T${photoNumber}.txt`), { cache: "no-store" });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    textByLanguage = parseBilingualText(await response.text());
  } catch (error) {
    textByLanguage = {
      fr: "Le texte de ce point de vue n’est pas disponible.",
      en: "The text for this viewpoint is not available."
    };
  }
}

async function openTextModal() {
  await loadTextForCurrentView();
  if (textModalTitle) textModalTitle.textContent = placeTitle?.textContent || `Vue ${photoNumber}`;
  if (textModalSubtitle) {
    textModalSubtitle.textContent = [placePrecision?.textContent, placeDate?.textContent].filter(Boolean).join(" · ");
  }
  renderTextLanguage(currentTextLanguage);
...
audioBtn?.addEventListener("click", toggleVoiceOver);
voiceOver?.addEventListener("ended", () => setAudioButtonState(false));
voiceOver?.addEventListener("pause", () => {
  if (voiceOver.currentTime === 0 || voiceOver.ended) setAudioButtonState(false);
});
...
textModal?.addEventListener("click", event => {
  if (event.target.closest("[data-close-text]")) closeTextModal();
});
tabFr?.addEventListener("click", () => renderTextLanguage("fr"));
tabEn?.addEventListener("click", () => renderTextLanguage("en"));
