...
const heritageTextContent = document.getElementById("heritageTextContent");
const textModalTitle = document.getElementById("textModalTitle");
const textModalPlace = document.getElementById("textModalPlace");
...
/* =========================================================
   V12.3 — AUDIOGUIDE + TEXTE BILINGUE
   audio/V1.mp3 … audio/V12.mp3
   textes/T1.txt … textes/T12.txt
   ========================================================= */

let heritageTexts = { fr: "", en: "" };
let currentHeritageLanguage = "fr";
let heritageTextLoadedFor = null;
let textModalPreviouslyFocused = null;

function stopVoiceAudio(reset = true) {
  ...
  audioBtn?.setAttribute("aria-label", "Écouter la voix off");
  if (audioBtn) audioBtn.title = "Écouter la voix off";
}

async function toggleVoiceAudio() {
  if (!voiceAudio || !audioBtn) return;
  ...
  const expectedSrc = assetUrl(`audio/V${photoNumber}.mp3`);
  if (voiceAudio.src !== expectedSrc) voiceAudio.src = expectedSrc;

  try {
    await voiceAudio.play();
    audioBtn.classList.add("is-playing");
    audioBtn.setAttribute("aria-label", "Arrêter la voix off");
    audioBtn.title = "Arrêter la voix off";
  } catch (error) {
    stopVoiceAudio(false);
    showError("Impossible de lire la voix off. Vérifiez que le fichier audio est bien présent.");
  }
}

function splitBilingualText(rawText) {
  const normalized = String(rawText || "").replace(/\r\n?/g, "\n").trim();
  if (!normalized) return { fr: "Texte indisponible.", en: "Text unavailable." };

  const blocks = normalized.split(/\n\s*\n+/).map(block => block.trim()).filter(Boolean);
  if (blocks.length >= 2) {
    return {
      fr: blocks[0],
      en: blocks.slice(1).join("\n\n")
    };
  }
...
async function loadHeritageText() {
  if (heritageTextLoadedFor === photoNumber) return heritageTexts;

  heritageTextLoadedFor = photoNumber;
  heritageTexts = { fr: "Chargement…", en: "Loading…" };

  try {
    const response = await fetch(assetUrl(`textes/T${photoNumber}.txt`), { cache: "no-store" });
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    heritageTexts = splitBilingualText(await response.text());
  } catch (error) {
    heritageTexts = {
      fr: "Le texte patrimonial n’est pas disponible pour ce point de vue.",
      en: "The heritage text is not available for this viewpoint."
    };
  }

  return heritageTexts;
}

function renderHeritageLanguage(language) {
  currentHeritageLanguage = language === "en" ? "en" : "fr";
  const isFrench = currentHeritageLanguage === "fr";

  langFrBtn?.classList.toggle("is-active", isFrench);
  langEnBtn?.classList.toggle("is-active", !isFrench);
  langFrBtn?.setAttribute("aria-selected", String(isFrench));
  langEnBtn?.setAttribute("aria-selected", String(!isFrench));

  if (heritageTextContent) {
    heritageTextContent.textContent = heritageTexts[currentHeritageLanguage] || "";
    heritageTextContent.setAttribute("lang", currentHeritageLanguage);
  }
}

async function openTextModal() {
  if (!textModal) return;
  textModalPreviouslyFocused = document.activeElement;
  textModal.hidden = false;
  textModal.setAttribute("aria-hidden", "false");
  document.body.classList.add("text-modal-open");

  if (textModalTitle) textModalTitle.textContent = currentHeritageLanguage === "en" ? "Heritage text" : "Texte patrimonial";
  if (textModalPlace) {
    const title = placeTitle?.textContent?.trim() || "";
    const date = placeDate?.textContent?.trim() || "";
    textModalPlace.textContent = [title, date].filter(Boolean).join(" — ");
  }

  heritageTexts = { fr: "Chargement…", en: "Loading…" };
  renderHeritageLanguage(currentHeritageLanguage);
  await loadHeritageText();
  renderHeritageLanguage(currentHeritageLanguage);
  closeTextBtn?.focus();
}
...
function closeTextModal() {
  ...
  if (textModalPreviouslyFocused && typeof textModalPreviouslyFocused.focus === "function") {
    textModalPreviouslyFocused.focus();
  }
}

audioBtn?.addEventListener("click", toggleVoiceAudio);
voiceAudio?.addEventListener("ended", () => stopVoiceAudio(true));
voiceAudio?.addEventListener("error", () => {
  stopVoiceAudio(false);
  showError("Fichier audio introuvable pour ce point de vue.");
});
...
textModal?.addEventListener("click", event => {
  if (event.target.hasAttribute("data-close-text-modal")) closeTextModal();
});
langFrBtn?.addEventListener("click", () => {
  if (textModalTitle) textModalTitle.textContent = "Texte patrimonial";
  renderHeritageLanguage("fr");
});
langEnBtn?.addEventListener("click", () => {
  if (textModalTitle) textModalTitle.textContent = "Heritage text";
  renderHeritageLanguage("en");
});

document.addEventListener("keydown", event => {
  if (event.key === "Escape" && textModal && !textModal.hidden) closeTextModal();
});
